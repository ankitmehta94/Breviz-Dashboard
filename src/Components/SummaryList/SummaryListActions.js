import {UPDATE_SUMMARY, CLICKED_TEXT, UPDATE_AGENDA, ADD_SUMMARY, SET_CREATION_LOADER} from '../../Constants/actionConstants';
...
//   import {disableLocationActiveState} from './hotelHelperFunctions';
  ...
  const getSummaryObject = (path) => async(dispatch, getState, {api}) => {
    ...
    if(dummySummaryObject){
      ...
      dispatch({
        type : ADD_SUMMARY,
        payload : {summaryList: dummySummaryObject, clickedText: nameOfSumamary, SummaryKey: nameOfSumamary}
      });
    ...
    dispatch({
      type : ADD_SUMMARY,
      payload : {summaryList: SummaryListObject, clickedText: '', SummaryKey: meetingName}
    });
    return response
  }
